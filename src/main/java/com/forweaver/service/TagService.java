...
public class TagService {
	...
	public boolean validateTag(List<String> tagList,Weaver weaver) {
		...
		if(userTags.size() >= 3) //메세지 태그가 세개 이상일 때
			return false;
		
		if(!myTag && userTags.size() == 2) //자신의 태그가 없고 메세지 태그가 두개 아닐 때
			return false;
		
		if (projectTags.size() >0 && userTags.size() > 0) // 권한을 가진 태그가 있고 메세지 태그가 있을때
			return false;
		
		if(userTags.size() > 0 && weaver != null)  // 메시지 태그가 있고 로그인한 사람의 경우
			return true;
		...
		for (Pass pass : weaver.getPasses()) // 권한 검증
			if (projectTags.size() > 0 && projectTags.get(0).equals("@" + pass.getJoinName())) 
				return true;
